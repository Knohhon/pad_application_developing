...
from app.models.database_models import OrderItem, Product
...
class ProductRepository:
    ...
    async def create(
        ...
        if "price" in product_dict:
            product_dict["price"] = Decimal(str(product_dict["price"])).quantize(
                Decimal("0.01")
            )
    ...
    async def get_low_stock_products(
        self, session: AsyncSession, threshold: int = 10, limit: int = 20
    ) -> List[Product]:
