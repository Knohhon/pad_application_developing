...
class TestProductRepository:
    ...
    async def test_update_product(self, session, product_repository):
        ...
        update_data = ProductUpdate(
            label="Updated Product", price=Decimal("15.99"), count_in_warehouse=150
        )
    ...
    async def test_get_products_list(
        ...
        products_data = [
            ProductCreate(
                label="Product A",
                count_in_package=5,
                count_in_warehouse=20,
                price=Decimal("5.99"),
            ),
            ProductCreate(
                label="Product B",
                count_in_package=10,
                count_in_warehouse=30,
                price=Decimal("9.99"),
            ),
            ProductCreate(
                label="Product C",
                count_in_package=15,
                count_in_warehouse=40,
                price=Decimal("12.50"),
            ),
        ]
        ...
        products = await product_repository.get_by_filter(
            session=session, count=10, page=1
        )
    ...
    async def test_delete_product(
        ...
        deleted = await product_repository.get_by_id(
            session=session, product_id=product.id
        )
