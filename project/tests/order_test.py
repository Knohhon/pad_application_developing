...
async def test_create_order_with_multiple_products(
    ...
    user = await user_repository.create(
        ...
        user_data=UserCreate(
            email="order@example.com", username="order_user", description="Test user"
        ),
    )
    ...
    order = await order_repository.create(
        session=session, order_data=order_data  # <-- Передаем реальную сессию
    )
    ...
    updated_product1 = await product_repository.get_by_id(
        session=session, product_id=product1.id  # <-- Передаем реальную сессию
    )
    updated_product2 = await product_repository.get_by_id(
        session=session, product_id=product2.id  # <-- Передаем реальную сессию
    )
