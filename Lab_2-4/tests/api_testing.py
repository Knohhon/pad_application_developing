...
from litestar.status_codes import HTTP_200_OK, HTTP_201_CREATED, HTTP_404_NOT_FOUND, HTTP_422_UNPROCESSABLE_ENTITY
...
class MockUserService:
    ...
    async def update(self, session: Any, user_id: UUID, user_data: UserUpdate) -> UserResponse:
        if self.user_response:
...
async def update_user(user_id: UUID, data: UserUpdate, service: MockUserService) -> UserResponse:
...
def test_get_user_endpoint(user_response: UserResponse):
    ...
    with create_test_client(
        ...
        dependencies={
            "service": Provide(
                lambda: MockUserService(user_response=user_response)
            )
        }
    ) as client:
...
def test_get_user_not_found():
    ...
    with create_test_client(
        ...
        dependencies={
            "service": Provide(
                lambda: MockUserService(user_response=None)
            )
        }
    ) as client:
...
def test_update_user_endpoint(user_response: UserResponse, user_update_data: UserUpdate):
